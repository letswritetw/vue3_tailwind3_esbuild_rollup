...
${codeFrame}` : message);
  ...
  // src/components/useDemoComponent.js
  var useDemoComponent_default = {
    props: {
      title: {
        type: String,
        default: ""
...
Only state can be modified.`);
  ...
  function createOptionsStore(id, options, pinia2, hot) {
    ...
    function setup() {
      if (!initialState && !hot) {
        if (isVue2) {
          set2(pinia2.state.value, id, state ? state() : {});
        } else {
    ...
    for (const key in setupStore) {
      ...
      if (isRef(prop) && !isComputed(prop) || isReactive(prop)) {
        if (hot) {
          ...
          if (isVue2) {
            set2(pinia2.state.value[$id], key, prop);
          } else {
...
Found in store "${store.$id}".`);
  ...
  function defineStore(idOrOptions, setup, setupOptions) {
    ...
    function useStore(pinia2, hot) {
      ...
      (false ? null : pinia2) || currentInstance2 && inject(piniaSymbol, null);
      if (pinia2)
        setActivePinia(pinia2);
      if (!activePinia) {
...
This will fail in production.`);
  ...
  // src/store/useDemoPinia.js
  var useDemoPinia = defineStore("useDemoPinia", () => {
    const count = ref(0);
    const addCount = () => {
      count.value++;
    };
  ...
  var App = {
    components: { useDemoComponent: useDemoComponent_default },
    setup() {
